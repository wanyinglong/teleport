	...
	if domainName == ca.GetClusterName() {
		...
		for _, u := range users {
			if u.GetName() == teleportUser {
				// pass along the context so we get the substituted roles for this user
				roles, err = services.FetchRoles(u.GetRoles(), s.authService, u.GetContext())
				if err != nil {
					return "", trace.Wrap(err)
				}
			}
		...
		// pass nil for the context as we don't want to allow substitution into
